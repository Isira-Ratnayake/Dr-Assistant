 ...
 * @author IU Ratnayake
 */
public class UMPatientGetway extends PrescriptionGetway {
    
        public Patient newestPatient() {
        ...
        try {
            pst = con.prepareStatement("select * from patient where id = (select max(id) from patient);");
            rs = pst.executeQuery();
            if (rs.next()) {
                patient = new Patient(
                        rs.getInt(1),
